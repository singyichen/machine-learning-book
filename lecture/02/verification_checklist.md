...
驗證對象：`02_assignment1.py` + `tech_employees.csv` + `assignment1_results.txt` + `assignment1_decision_boundaries.png`
...
**總結**：核心程式邏輯（資料切分、標準化無洩漏、SGD 訓練收斂、正確率計算、特徵組合比較、決策邊界視覺化）皆通過驗證，結果與 PDF 預期趨勢相符；新增的 loss 收斂曲線與自適應學習率對比圖也已產出並驗證趨勢合理。
